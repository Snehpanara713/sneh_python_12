...
def delete_data():
    if e_id.get()=="":
        msg.showinfo("Delete Status","ID Is Mandatory For Delete")
    else:
        conn=create_conn()
        cursor=conn.cursor()
...
insert=Button(root,text="INSERT",bg="black",fg="white",font=("Elephant",10),command=insert_data)
insert.place(x=50,y=300)

search=Button(root,text="SEARCH",bg="black",fg="white",font=("Elephant",10),command=search_data)
search.place(x=130,y=300)

update=Button(root,text="UPDATE",bg="black",fg="white",font=("Elephant",10),command=update_data)
update.place(x=214,y=300)

delete=Button(root,text="DELETE",bg="black",fg="white",font=("Elephant",10),command=delete_data)
delete.place(x=298,y=300)
